...
public class TVList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		pw.print("<h2 style='font-weight: bolder;'>Results for "+maker1+" TVs.....</h2>");
		...
		try{
			...
			while(rs.next()){
				...
				pw.print("<form method='post' action='AddToCart'><img src='"+rs.getString("p_image")+"' style='width: 25%'><input type='hidden' name='userName' value='"+username+"'/><input type='hidden' name='id' value='"+rs.getString("p_id")+"'/><input type='hidden' name='sku' value='"+rs.getString("p_sku")+"'/><input type='hidden' name='type' value='tvs'/><input type='hidden' name='image' value='"+rs.getString("p_image")+"'/><input type='hidden' name='model' value='"+rs.getString("p_model")+"'/><input type='hidden' name='price' value='"+afterDiscount+"'/><input type='hidden' name='manufacturer' value='"+rs.getString("p_manufacturer")+"'/><input type='hidden' name='name' value='"+rs.getString("p_name")+"'/>");
